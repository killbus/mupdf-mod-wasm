...
export class PDFDocument extends Document {
	...
	/**
	 * Save PDF document with streaming output for memory-efficient export.
	 *
	 * Instead of buffering the entire PDF in memory, this method calls the
	 * onChunk callback incrementally as data is generated (~8KB chunks).
	 *
	 * @param onChunk Callback invoked with each data chunk. Called with null when complete.
	 * @param options Write options (same as saveToBuffer)
	 * @returns Callback ID (for internal use)
	 *
	 * @example
	 * ```typescript
	 * const callbackId = doc.saveToBufferStreaming(
	 *   (chunk) => {
	 *	 if (chunk) {
	 ...
	 * ```
	 */
	saveToBufferStreaming(onChunk, options = "") {
		var options_string;
		if (typeof options === "object") {
			options_string = Object.entries(options).map(kv => {
				var k = kv[0];
		...
		// Register callback in global storage
		$streaming_callbacks[callbackId] = onChunk;
		try {
			// Call WASM streaming export function
			libmupdf._wasm_pdf_write_document_streaming(this.pointer, STRING(options_string), callbackId);
			// Send end-of-stream signal
			onChunk(null);
		}
		finally {
			// Cleanup callback registration
			delete $streaming_callbacks[callbackId];
		}
		return callbackId;
	}
...
/* -------------------------------------------------------------------------- */
// Storage for streaming export callbacks
var $streaming_callbacks = {};
globalThis.$libmupdf_streaming_callback = function (callbackId, dataPtr, size) {
	const callback = $streaming_callbacks[callbackId];
	if (callback) {
		// Copy WASM memory to JavaScript
		// slice() creates a copy so WASM can reuse the buffer
		const chunk = libmupdf.HEAPU8.slice(dataPtr, dataPtr + size);
		callback(chunk);
	}
};
